...
|:---|:---:|:---:|:---:|:---:|:---:|
| **技术栈** | pywebview 原生 | Electron | Docker + Node | Next.js | Electron |
| **安装体积** | **< 1 MB** | 200 MB+ | 500 MB+ | 300 MB+ | 150 MB+ |
| **运行内存** | **~30 MB** | ~200 MB+ | ~500 MB+ | ~300 MB+ | ~150 MB+ |
| **启动速度** | **< 2 秒** | 5-10 秒 | 需 Docker | 需 Node | 5-8 秒 |
| **框架依赖** | **零** | Electron | Node.js | React | Electron |
| **系统集成** | 原生窗口 | 伪原生 | 浏览器 | 浏览器 | 伪原生 |

> **Hermes Pulse 用一个 Python 脚本 + 三个前端文件，实现了 Electron 200MB 才能做到的事。**
> 这不是"简陋"，这是"精准"——只做该做的事，把每一份资源用在刀刃上。
...
从 [Releases](https://github.com/MINTSOLD/hermes-gui/releases) 下载最新版本，双击安装即可。
...
4. 使用 `start_config_server.vbs` 包装器隐藏控制台窗口
...
- 🐛 **报告 Bug**：[提交 Issue](https://github.com/MINTSOLD/hermes-gui/issues)
- 💡 **功能建议**：[发起 Discussion](https://github.com/MINTSOLD/hermes-gui/discussions)
- 🔧 **提交代码**：Fork → Branch → PR
- ⭐ **给我们 Star**：这是最简单的支持方式
